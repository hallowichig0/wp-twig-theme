...
class Bootstrap4Action extends Timber\Site {
	...
	public function __construct() {
		...
		add_action( 'wp_head', array($this, 'favicon_default') );
		add_action( 'pre_get_posts', array($this, 'add_cpt_in_author' ) );
		add_action( 'pre_get_posts', array($this, 'add_cpt_in_archive') );
		// add_action( 'pre_get_posts', array($this, 'add_cpt_in_archive'));
		add_action( 'get_header', array($this, 'remove_margin_in_admin_login_header') );
		add_action( 'wp_head', array($this, 'bootstrap4_pingback_header'));
		add_action( 'wp_body_open', array($this, 'show_template') );
		add_action( 'init', array($this, 'disable_emoji_feature') );
		add_action( 'admin_init', array($this, 'remove_menu_pages') );
		add_action( 'admin_head', array($this, 'hide_appearance_submenu'), 999 );
		add_action( 'customize_register', array($this, 'remove_customize_register'), 100 );
		// add_action( 'admin_init', array($this, 'remove_comment' ));
	 ...
	 * Default Favicon
	 */
	public function favicon_default() {
	 ...
	 */
	public function add_cpt_in_author( $query ) {
	 ...
	 */
	public function add_cpt_in_archive( $query ) {
	 ...
	 */
	public function remove_margin_in_admin_login_header() {
	 ...
	 */
	public function bootstrap4_pingback_header() {
	...
	*/
	 ...
	 */
	public function disable_emoji_feature() {
	 ...
	 */
	public function hide_appearance_submenu() {
	 ...
	 */
	public function remove_customize_register( $wp_customize ) {
		// check condition for the user means show menu for this user
		if( !current_user_can('administrator') ) {
